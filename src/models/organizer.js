...
export const meta = {
  name: "Grid Organizer",
  params: [
    { key: "width", label: "Width", min: 40, max: 300, default: 120 },
    { key: "depth", label: "Depth", min: 40, max: 300, default: 80 },
...
export function build(oc, params) {
  ...
  // 2. Targeted Vertical Fillets (Exterior)
  if (p.fillet_r > 0.1) {
    try {
  ...
  // 3. Subtract Cavity
  const cav = makeBox(oc, p.width - 2 * p.wall_t, p.depth - 2 * p.wall_t, p.height);
  ...
  // 4. Grid Dividers
  const rowStep = (p.depth - 2 * p.wall_t) / p.rows;
  const colStep = (p.width - 2 * p.wall_t) / p.cols;

  for (let i = 1; i < p.rows; i++) {
    const div = makeBox(oc, p.width - 2 * p.wall_t, p.wall_t, p.height - p.wall_t);
  ...
  for (let j = 1; j < p.cols; j++) {
    const div = makeBox(oc, p.wall_t, p.depth - 2 * p.wall_t, p.height - p.wall_t);
    const tr = new oc.gp_Trsf_1();
