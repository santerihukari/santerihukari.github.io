...
export function buildPortableHangboardBrep(oc, params) {
  ...
  // 6. THE FLIP (Rotate 180 degrees around X axis to make slot upward)
...
function makePrismAt(oc, x, y, z, dx, dy, dz) {
  ...
  const p1 = new oc.gp_Pnt_3(x + dx, y, z);
  const p2 = new oc.gp_Pnt_3(x + dx, y + dy, z);
  const p3 = new oc.gp_Pnt_3(x, y + dy, z);

  const poly = new oc.BRepBuilderAPI_MakePolygon_1();
  [p0, p1, p2, p3].forEach(p => poly.Add_1(p));
  poly.Close();
...
function makeLoftedCap(oc, d) {
  ...
  mk.AddWire(w0);
  mk.AddWire(w1);
  mk.Build(safeNewProgressRange(oc) || new oc.Message_ProgressRange());
  return mk.Shape();
}
