export const meta = {
  ...
  params: [
    ...
    { key: "outer_vertical_fillet_r", label: "Outer vertical fillet", min: 0, max: 12, default: 0.0 },
    { key: "front_horizontal_fillet_r", label: "Front horizontal fillet", min: 0, max: 12, default: 0.0 },
    { key: "internal_riser_fillet_r", label: "Internal riser fillet", min: 0, max: 6, default: 0.0 },
...
export function build(oc, params) {
  ...
  if (p.slot_edge_fillet_r > 0.05) {
    shape = filletSlotEdgesSafest(
      oc,
      shape,
      p.slot_edge_fillet_r,
      slotXStart,
      slotWidthX,
      p.bottom_wall_z,
      slotHeight,
...
function filletSlotEdgesSafest(oc, shape, radius, slotXStart, slotWidthX, slotZ0, slotHeight, slotDepthY, sideWallX, blockWidthX) {
  const tries = [radius, 0.8 * radius, 0.6 * radius, 0.4 * radius].filter((v, i, a) => v > 0.05 && a.indexOf(v) === i);
  ...
  for (const r of tries) {
    try {
      ...
      while (exp.More()) {
        ...
        const insideSlotX = c.X() > slotXStart + 0.6 && c.X() < slotXStart + slotWidthX - 0.6;
        const nearFront = c.Y() > -0.6 && c.Y() < 0.8;
        const nearBack = c.Y() > slotDepthY - 0.8 && c.Y() < slotDepthY + 0.8;
        const inWorkingDepth = nearFront || nearBack;
        const insideSlotBandZ = c.Z() > slotZ0 + 0.6 && c.Z() < slotZ0 + slotHeight - 0.6;
        const notNearOuterWalls = c.X() > sideWallX - 0.2 && c.X() < blockWidthX - sideWallX + 0.2;

        if (insideSlotX && inWorkingDepth && insideSlotBandZ && notNearOuterWalls) {
          mk.Add_2(r, edge);
          count++;
        }
        exp.Next();
      ...
      // try a smaller radius below
    }
  }

  console.warn('Slot edge fillet failed. Try reducing slot_edge_fillet_r, increasing slot_height/clearance, or reducing boolean_fuzzy.');
  return shape;
}
